...
export default function AdminDashboard() {
  ...
  const handleAddClass = async () => {
    ...
    try {
      const payload={grade:selGrade.id,name:newClass.name.trim()};
          ...
          {tab==="timetable"&&(
            <div className="fade">
              <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:18}}>
                {/* LEFT — Class List */}
                <div>
                  <h3 style={{fontSize:14,fontWeight:700,color:"#0F172A",marginBottom:12}}>Select Class</h3>
                  {allClasses.length===0?(
                    <div className="card" style={{textAlign:"center",color:"#94A3B8",padding:32}}>
                      <div style={{fontSize:28,marginBottom:6}}>🏛️</div>
                      <p style={{fontSize:13}}>No classes yet. Create classes in Grades & Classes first.</p>
                    </div>
                  ):allClasses.map(c=>(
                    <div key={c.id}
                      className={`sel-item${selTTClass?.id===c.id?" active":""}`}
                      onClick={()=>setSelTTClass(c)}>
                      <div style={{fontWeight:700,fontSize:13,color:"#0F172A"}}>
                        Class {c.name} <span style={{color:"#64748B",fontWeight:400}}>({c.grade_name})</span>
                      </div>
                      <div style={{fontSize:12,color:"#64748B",marginTop:2}}>
                        {c.student_count||0} students · Teacher: {c.teacher_name||"Unassigned"}
                      </div>
                    </div>
                  ))}
                </div>

                {/* RIGHT — Timetable Manager */}
                <div>
                  {!selTTClass?(
                    <div className="card" style={{textAlign:"center",color:"#94A3B8",padding:50}}>
                      <div style={{fontSize:40,marginBottom:12}}>👈</div>
                      <p style={{fontSize:13}}>Select a class to view & manage its timetable</p>
                    </div>
                  ):(
                    <div className="card">
